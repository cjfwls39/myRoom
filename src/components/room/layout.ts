...
export const ORBIT = {
  ...
  maxDistance:     60,    // 축소 한계 — 충분히 멀리 당길 수 있도록
...
export const POS = {
  ...
  // ── 고양이 액자 그룹 (LeftWall 로컬) ─────────────────
  catFrames: [-INNER + 0.02, 2.95,  -1.30] as [number, number, number],
...
export const SIZE = {
  ...
  //  → 90도 회전 후 모니터 화면(+Z 법선)이 카메라 방향(+X/+Z)을 향함 ✓
  ...
  //  아크릴 패널(+X 법선)이 90도 회전 후 카메라를 향함 ✓
  ...
  //  Desk.tsx에서 rotation={[0, PI, 0]} 추가 180도 → 등받이가 카메라 반대(안쪽)를 향함 ✓
