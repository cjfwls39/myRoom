...
export const SIZE = {
  ...
  // ── PC 본체 ───────────────────────────────────────
  //  책상 상판 위에 세워짐
  //  아크릴 패널이 카메라 방향(+X)을 향함
  //  내부 좌표: X=두께방향 / Y=높이 / Z=폭
  pc: {
    // 케이스 외형
    sizeX:    0.38,   // 두께 (카메라 방향)
    sizeY:    0.88,   // 높이
    sizeZ:    0.92,   // 폭
    wallT:    0.012,  // 벽 두께
    ...
    gpuH:    0.16,    // 높이
    gpuD:    0.68,    // 폭 (Z)
    gpuX:    0.04,    // 케이스 중심 기준 X
    // 앞면 팬 (3개)
    fanSize:  0.155,  // 팬 한 변
    fanT:     0.020,  // 두께
    fanYs:   [0.20, 0.42, 0.64] as number[],  // Y 위치
    fanR:     0.060,  // 팬 블레이드 반지름
    // 상단 팬 (2개)
    topFanSize: 0.135,
