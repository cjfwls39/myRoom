...
export const SIZE = {
  ...
  // ── PC 본체 (책상 상판 위) ───────────────────────────
  //  PcTower group rotation=[0,PI,0] 적용 후 내부 좌표:
  //    X: - = 메인보드 벽(뒷면) / + = 아크릴 패널(카메라쪽)
  //    Y: 0 = 바닥 / sizeY = 천장
  //    Z: - = 케이스 뒤 / + = 케이스 앞(팬면)
  //  모든 부품 좌표는 케이스 로컬(0,0,0) 기준
  pc: {
    // ── 케이스 외형 ───────────────────────────────────
    sizeX:    0.38,   // 두께 (X, 카메라 방향)
    sizeY:    0.88,   // 높이 (Y)
    sizeZ:    0.92,   // 폭   (Z)
    wallT:    0.012,  // 벽 두께
    offsetX:  1.10,   // 책상 중심 기준 X
    offsetZ: -0.10,   // 책상 중심 기준 Z
    // ── 아크릴 패널 ──────────────────────────────────
    acrylicOpacity:   0.18,
    acrylicRoughness: 0.50,
    // ── 메인보드 ──────────────────────────────────────
    // 왼쪽 벽(-X) 안쪽에 세워짐, 모든 부품은 mbOffX 기준으로 X를 계산
    mbOffX:  -0.14,   // 케이스 중심에서 메인보드까지 X 오프셋
    mbH:      0.64,   // 메인보드 높이
    mbD:      0.78,   // 메인보드 폭 (Z)
    mbBaseY:  0.15,   // 메인보드 시작 Y (PSU 위)
    // ── CPU ───────────────────────────────────────────
    // 메인보드 중앙 상단에 장착
    cpuOffY:  0.46,   // 케이스 기준 절대 Y
    cpuOffZ:  0.08,   // 케이스 기준 Z (중앙 약간 위)
    cpuSize:  0.08,   // CPU IHS 한 변
    // ── 수냉 헤드 ─────────────────────────────────────
    wcHeadW:  0.09,   // 헤드 X 두께
    wcHeadH:  0.08,   // 헤드 높이
    wcHeadD:  0.09,   // 헤드 Z 폭
    // ── 수냉 라디에이터 (케이스 상단 안쪽) ──────────
    radOffY:  0.72,   // 케이스 기준 Y (천장 아래로 내림)
    radW:     0.30,   // 라디에이터 Z 폭
    radH:     0.022,  // 두께
    radFinCount: 10,  // 핀 개수
    // ── 수냉 튜브 ─────────────────────────────────────
    tubeR:    0.009,  // 반지름
    tubeZOff: 0.055,  // 튜브 Z 간격 (±)
    // ── GPU ───────────────────────────────────────────
    // 메인보드에 수직으로 꽂힌 카드 형태 — X방향으로 얇게
    gpuOffY:  0.27,   // 케이스 기준 Y (GPU 중심)
    gpuThick: 0.016,  // GPU PCB 두께 (X)
    gpuH:     0.22,   // GPU 높이 (Y) — 쿨러 포함
    gpuD:     0.72,   // GPU 폭 (Z)
    gpuShiftX: 0.06,  // mbOffX에서 +X 방향으로 얼마나 돌출
    gpuFanR:  0.036,  // GPU 팬 반지름
    gpuFanZs: [-0.22, 0.00, 0.22] as number[], // 팬 3개 Z 위치
    // ── RAM (4개) ─────────────────────────────────────
    // 메인보드 표면 위에 수직으로 서있음
    ramOffY:  0.40,   // 케이스 기준 Y (RAM 하단)
    ramOffZ: -0.20,   // 첫 번째 RAM Z (메인보드 중앙 왼쪽)
    ramW:     0.007,  // 두께 (X)
    ramH:     0.13,   // 높이 (Y)
    ramGap:   0.028,  // RAM 간격 (Z)
    // ── NVMe SSD ──────────────────────────────────────
    ssdOffY:  0.24,   // 케이스 기준 Y
    ssdOffZ:  0.22,   // 케이스 기준 Z
    ssdW:     0.007,  // 두께 (X)
    ssdH:     0.018,  // 높이 (Y)
    ssdD:     0.076,  // 길이 (Z)
    // ── PSU (하단) ────────────────────────────────────
    ...
    // ── 상단 라디에이터 팬 2개 ────────────────────────
    topFanSize: 0.130,
    topFanZs: [-0.075, 0.075] as number[],
    // ── 케이블 다발 ───────────────────────────────────
    cableOffY: 0.14,  // PSU 위쪽
  },
